...
public class BoardTest extends TestCase {
	public void testSingleDistance() {
		assertEquals(1,
				Board.distance(new Player[0], new Point(0, 7), new Point(1, 7)));
	}

	public void testMoveToRoom() {
		assertEquals(4, Board.distance(new Player[0], new Point(2, 16),
				new Point(6, 15)));
	}
	...
	public void testMoveAroundRoom() {
		assertEquals(0, Board.distance(new Player[0], new Point(1, 14),
				new Point(6, 11)));
	}

	public void testKitchenToStudy() {
		assertEquals(1, Board.distance(new Player[0], new Point(1, 3),
				new Point(18, 22)));
	}

	public void testStudyToKitchen() {
		assertEquals(1, Board.distance(new Player[0], new Point(18, 22),
				new Point(1, 3)));
	}

	public void testConservatoryToLounge() {
		assertEquals(1, Board.distance(new Player[0], new Point(22, 2),
				new Point(3, 21)));
	}

	public void testLoungeToConservatory() {
		assertEquals(1, Board.distance(new Player[0], new Point(3, 21),
				new Point(22, 2)));
	}
	...
	public void testPlayersBlockEntrance() {
		...
		assertEquals(2, Board.distance(
				players.toArray(new Player[players.size()]), new Point(6, 17),
				new Point(6, 14)));
		Player p = new Player(new User("a", "test player"), Suspect.SCARLETT, 0);
		p.setPosition(new Point(6, 15));
		players.add(p);
		assertEquals(-1, Board.distance(
				players.toArray(new Player[players.size()]), new Point(6, 17),
				new Point(6, 14)));
